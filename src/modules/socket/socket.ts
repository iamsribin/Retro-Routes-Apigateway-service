...
export const setupSocketIO = (server: HttpServer): SocketIOServer => {
  ...
  console.log(
    `Socket.IO initialized with CORS origin: ${process.env.CORS_ORIGIN}`
  );
...
const handleSocketConnection = (
  ...
  socket.on("register", (data: { userId: string; role: string }) => {
    if (data.userId && data.userId !== "undefined") {
      ...
      console.log(
        `Re-registered ${data.role} ${
          data.userId
        } in userSocketMap: ${JSON.stringify(userSocketMap)}`
      );
    }
  });
...
const setupAdminEvents = (
  ...
  socket.on("block-user", ({ userId: targetUserId }: { userId: string }) => {
    console.log(
      `Received block-user event for targetUserId: ${targetUserId} from admin: ${userId}`
    );
    const targetSocketId = userSocketMap[targetUserId];

    if (targetSocketId) {
      console.log(
        `Emitting user-blocked event to socket: ${targetSocketId} for user: ${targetUserId}`
      );
      io.to(targetSocketId).emit("user-blocked");
    } else {
      console.error(
        `No socket found for user: ${targetUserId} in userSocketMap`
      );
      socket.emit("error", {
...
const setupDriverEvents = (
  ...
  socket.on("driverOffline", async () => {
    ...
    try {
      ...
      socket.emit("error", {
        message: "Failed to remove driver from cache",
        code: "DRIVER_OFFLINE_FAILED",
      });
    }
  });
...
const updateDriverLocation = async (
  ...
  if (inRideDriverExists) {
    const inRideDataRaw = await redisClient.get(inRideDriverDetailKey);
    if (!inRideDataRaw) return;

    const inRideData = JSON.parse(inRideDataRaw);
    const userId = inRideData.userId;
  ...
  socket.on(
    ...
    async (rideData: {
      ...
      try {
        ...
        console.error(
          `Error processing ride request for user ${userId}:`,
          error
        );
...
console.log("drivers",drivers);
  ...
  await handleDriverRideRequests(
    socket,
    io,
    userId,
    ride,
...
const handleDriverRideRequests = async (
  ...
  for (const driver of drivers) {
    if (stopSignal.stop) {
      console.log(
        `Ride request processing stopped for ride ${ride.data.booking.ride_id}`
      );
      break;
    }

    const driverRideRequest: DriverRideRequest = createRideRequestData(
      userId,
      ride,
      rideData
    );
    ...
    const accepted = await sendRideRequest(
      io,
      driver.driverId,
      driverRideRequest,
      stopSignal
    ...
    if (accepted) {
      console.log(
        `Ride ${driverRideRequest.ride.rideId} accepted by driver ${driver.driverId}`
      );
...
const sendRideRequest = (
  ...
  return new Promise((resolve) => {
    ...
    const driverSocket = Array.from(io.sockets.sockets.values()).find(
      (s: AuthenticatedSocket) =>
        s.decoded?.clientId === driverId && s.decoded?.role === "Driver"
    );
    ...
    const responseHandler = async (response: {
      ...
      if (response.rideId !== rideRequest.ride.rideId) {
        console.log(
          `Response ride ID mismatch. Expected: ${rideRequest.ride.rideId}, Got: ${response.rideId}`
        );
        return;
      }
      ...
      driverSocket.off(
        `rideResponse:${rideRequest.ride.rideId}`,
        responseHandler
      );

      if (response.accepted) {
        await handleRideAcceptance(
          io,
          driverId,
          response.bookingId,
          response.rideId,
          rideRequest.customer.id,
          response.estimateTime
        );
        console.log(
          `Ride ${response.rideId} successfully accepted by driver ${driverId}`
        );
      } else {
    ...
    const timeout = setTimeout(async () => {
      console.log(
        `Ride request timeout for driver ${driverId}, ride ${rideRequest.ride.rideId}`
      );
      driverSocket.off(
        `rideResponse:${rideRequest.ride.rideId}`,
        responseHandler
...
const handleRideAcceptance = async (
  ...
  try {
    ...
    const parseEstimateTime = (timeStr: string): number => {
      ...
      return hours * 3600 + mins * 60 + 5 * 3600;
    };

    const ttlInSeconds = parseEstimateTime(estimateTime);
    console.log("ttlInSeconds==", ttlInSeconds);

    await redisClient.set(
      `inRide:driver:${driverId}`,
      JSON.stringify({
        rideId,
        bookingId,
        userId,
    ...
    const bookingResponse = (await bookingRabbitMqClient.produce(
      data,
      "accepted-booking"
    )) as BookingInterface;
    console.log(
    ...
    if (targetSocketId) {
      io.to(targetSocketId).emit("rideStatus", {
        ...
        driverCoordinates,
        driverDetails,
        userDetails: bookingResponse.data.user,
      });
    } else {
      console.error(`No socket found for user ${userId} in userSocketMap`);
...
const incrementDriverCancelCount = async (driverId: string) => {
  try {
    ...
    console.error(
      `Error incrementing cancellation count for driver ${driverId}:`,
      error
    );
  }
};

const setupDisconnectEvents = (
  socket: AuthenticatedSocket,
  io: SocketIOServer,
  role: string,
  userId: string
  ...
  socket.on("disconnect", async () => {
    ...
    if (userId && userId !== "undefined") {
      ...
      console.log(
        `Updated userSocketMap after disconnect: ${JSON.stringify(
          userSocketMap
        )}`
      );
    }
