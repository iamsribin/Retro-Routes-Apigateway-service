...
export default class driverAuthController {
  ...
  checkGoogleLoginDriver = async (
    ...
    try {
      ...
      const response: AuthResponse = (await driverRabbitMqClient.produce(
        { email },
        operation
      )) as AuthResponse;
  ...
  identificationUpdate = async (
    ...
    try {
      ...
      const response: Message = (await driverRabbitMqClient.produce(
        { ...req.body, ...req.query, aadharImageUrl, licenseImageUrl },
        operation
      )) as Message;
      res.status(StatusCode.Created).json(response);
    } catch (e: any) {
  ...
  vehicleUpdate = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const response: Message = (await driverRabbitMqClient.produce(
        { ...req.body, ...req.query, rcImageUrl, carImageUrl },
        operation
      )) as Message;
      res.status(StatusCode.Created).json(response);
    } catch (e: any) {
